...
# MEDIA_PATH = settings.SETTING_PATH + '/' + settings.['default']['NAME']+
# '/media/css'
...
def getListData(request, models):
    ...
    sortCol = '-name' if sortCol != '' and sortCol == request.session.get(
        'sortCol', '') else 'name'
...
def MasterDelete(request):
    ...
    try:
        if (request.method == 'POST'):
            ...
            dynamicForm = request.POST.get('masterType', 'DomainForm')
            modelName = dynamicForm.strip('Form')
            action = 'Delete_Success' if len(
                DELETE_METHOD[dynamicForm](masterIDS)) > 0 else 'Delete_UnSuccess' if (
                masterIDS != []) else 'SelectRecord'
...
#    if action == 'Save':
#        return HttpResponseRedirect('/projects/initiation/?msg=' + message) \
#            if project.strip('') == '' else \
#            HttpResponseRedirect('/projects/update/?ids='+project+'&msg='+message)
#    else:
#        pkID = request.POST.get('pkID', '')
#        return render(request, 'Create_Tag_Or_ProjectType.html', {
