...
class createics:
	...
	def __createICS__(self):
		try:			
			...
			item.startTime, item.eventDt = getEventTimes(self.request,item.startTime, item.eventDt, True)
			item.endTime, item.eventDt = getEventTimes(self.request,item.endTime, item.eventDt, False)			
			...
			for each in self.sendingIdsPwd:
				...
				if len(user)>0:
					...
					user[0].save()
					email_message = settings.EMAIL_MESSAGE %(user[0].email ,pwd)
				email_message = email_message+'\n'+item.message								
				Email().send_email(item.name, email_message, [each,], '', eventpath)							
		except Exception:			
	...
	def mailIDs(self, eventid, otherids):
		...
		sendingIDSPWD = []
		if (len(eventusers) > 0):
			fundausers = [FundaUser.objects.filter(pk = each.user_id) for each in eventusers]			
			userprofiles = [UserProfile.objects.get(pk = each[0].userProfile_id) for each in fundausers]			
			users = [User.objects.get(pk = each.authUser_id) for each in userprofiles]       
			emailID = '%s'
			for each in users:
