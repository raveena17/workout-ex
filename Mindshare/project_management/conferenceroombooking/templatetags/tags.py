...
def getTaskApprovalColor(task):
    ...
    for approver in taskApprover:
        ...
        if (taskCmd.approvedDate is None) or (
                taskCmd.approvedDate == GetDateType('2000-01-01')):
            app_msg = 'inprogress'
...
def tagtaskliststatus(tasklistid):
    status = False
    listdetails = ProgramTasklistDetail.objects.filter(
        programTasklist=tasklistid)

    if (len(listdetails) > 0):
        for each in listdetails:
            programtask = ProgramTask.objects.filter(
                pk=each.programTask_id, cancel=1)
            if len(programtask) <= 0:
                tasks = TaskAllocation.objects.filter(
                    programTask=each.programTask)
                if(len(tasks) > 0):
                    tasks = tasks.filter(status='Incomplete')
                    if(len(tasks) > 0):
                        status = False
