    ...
    def render(self, context):
        ...
        if cls is None:
            raise template.TemplateSyntaxError(
                _('full_tree_for_model tag was given an invalid model: %s') %
                self.model)
    ...
    def render(self, context):
        ...
        if self.foreign_key is not None:
            ...
            try:
                ...
                raise template.TemplateSyntaxError(
                    _('drilldown_tree_for_node tag was given an invalid model field: %s') %
                    fk_attr)
...
def do_full_tree_for_model(parser, token):
    ...
    if bits[2] != 'as':
        raise template.TemplateSyntaxError(
            _("second argument to %s tag must be 'as'") %
            bits[0])
...
def do_drilldown_tree_for_node(parser, token):
    ...
    if len_bits not in (4, 8, 9):
        raise TemplateSyntaxError(
            _('%s tag requires either three, seven or eight arguments') %
            bits[0])
    if bits[2] != 'as':
        raise TemplateSyntaxError(
            _("second argument to %s tag must be 'as'") %
    ...
    if len_bits == 8:
        ...
        return DrilldownTreeForNodeNode(
            bits[1], bits[3], bits[6], bits[8], cumulative=True)
    else:
