...
class ProjectBudget(models.Model):
    ...
    pjt_owner = models.ForeignKey(User, related_name="request_given_by",
                                  null=True)
    business_head = models.ForeignKey(
        User,
        related_name="going_to_approve",
        blank=True,
        null=True)
    status = models.ForeignKey(
        SaveRecordStatus,
        related_name="%(app_label)s_%(class)s_Record_status",
        blank=True,
        null=True)
...
class ProjectBudgetEfforts(models.Model):
    ...
    phase = models.ForeignKey(
        BudgetPhase,
        related_name="budgetphase_phase",
        blank=True,
        null=True)
    module = models.CharField(max_length=200, blank=True, null=True)
    location = models.ForeignKey(
        BudgetLocation,
        related_name="budgetlocation_location",
        blank=True,
        null=True)
