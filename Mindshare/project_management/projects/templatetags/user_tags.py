...
# Decorator to ensure only string is passed in
...
def get_internal_assigned_users(resources, role):
    return resources.filter(member__sysuserType="Internal").filter(roles=role)


@register.filter(name='get_external_assigned_users')
def get_external_assigned_users(resources, role):
    return resources.filter(member__sysuserType="External").filter(roles=role)
