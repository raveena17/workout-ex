        ...
        if form.is_valid():
            ...
            if flg != 1:
                ...
                repository = Repository.objects.create(
                    fileID=randomID, program_id=ID, repositoryTag_id=repositoryTag)
                ...
                fd = open(
                    '%s/%s' %
                    (currentosdir + ID, randomID + filename), 'wb')
                ...
                msg = 'Save'
                CapturLog().LogData(
                    request, msg, MODULE, actionMsg[msg], repository)
                form = FileUploadForm()
            else:
