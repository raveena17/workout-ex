...
jQuery.fn.toChecklist = function(o) { // "o" stands for options
	...
	this.each(function() {
		...
		if (o.addScrollBar) {
			$(checklistDivId).height(200 - findInListDivHeight).width(w);
		} else {
...
/* events for change in project */
/*configured for the use in pay it timsheet*/
var checklistDivId = '#id_tasks_checklist'

/* to check and un check the list box */
var check = function(event) {
        // This needs to be keyboard accessible too. Only check the box if the user
        // presses space (enter typically submits a form, so is not safe).
        if (event.type == 'keydown') {
...
var toggleDivGlow = function() {
        // Make sure the div is glowing if something is checked in it.			
        if ($('li',checklistDivId).hasClass('checked')) {
            $(checklistDivId).addClass('checklistHighlighted');
        } else {
            $(checklistDivId).removeClass('checklistHighlighted');
        }
    }
