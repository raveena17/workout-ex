...
class SubTaskForm(forms.Form):
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(self.__class__, self).__init__(*args, **kwargs)
        self.fields['sub_tasks'].queryset = Task.objects.filter(
            parent=task) if task else Task.objects.none()
...
class NonProjectTaskForm(forms.ModelForm):
    ...
    start_date = forms.DateField(input_formats=[DATE_INPUT_FORMAT],
                                 required=False, widget=forms.TextInput(attrs=DATE_FIELD_ATTR))
    # start_date = DateTimeField(input_formats=["%d %b %Y %H:%M:%S %Z"])

    # start_date = forms.DateField(('%d/%m/%Y',), label='Start date', required=False,
