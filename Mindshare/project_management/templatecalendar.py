...
def month_cal(year, month, request=None):
    ...
    event_list = BookConference.objects.filter(
        meeting_date__year=year,
        meeting_date__month=month).exclude(
        is_approved=0)  # eventDt__month=month
    for each in apex_body_owner:
        if each.username == str(request.user):
            event_list = BookConference.objects.filter(
                meeting_date__year=year, meeting_date__month=month).all()
    ...
    for each in event_list:
        dummystartTime, each.meeting_date = getEventTimes(
            request, each.from_time, each.meeting_date, True)
...
def getToday(request):
    gmtime = time.gmtime()
    data = str(gmtime.tm_hour) + ':' + \
        str(gmtime.tm_min) + ':' + str(gmtime.tm_sec)
...
def __getTimezone__(request):
    ...
    if COUNTRYTIMEZONE.__contains__(data):
        ...
        FROMTIMEZONE = 'Etc/GMT+' + \
            str(abs(offset)) if (offset > 0) else 'Etc/GMT-' + str(abs(offset))
