...
def task_tracking(request):
    ...
    if request.method == 'POST':
        ...
        date_text = request.POST.get(
            u'date', datetime.today().strftime(date_format))
        if request.is_ajax():
...
#        selected_tasks1 = Task.objects.filter(id__in = cache.get(request.user.pk, []))
        selected_tasks = TaskTracking.objects.filter(
            user=request.user).order_by('-start_time')[:15]
        task_name = Task.objects.filter(
            id__in=[
                selTask.task for selTask in selected_tasks],
...
def task_lookup(request):
    ...
    try:
        ...
        # return HttpResponse(simplejson.dumps(result), mimetype =
        # "application/json")
...
#        return HttpResponseNotFound(pagenotfound_msg)
    ...
    time_spent = TaskTracking.objects.filter(
        start_time__range=(
            date,
            next_day)).filter(
    ...
    # 'application/json')
    return HttpResponse(simplejson.dumps(time_spent),
                        content_type='application/json')
...
#        return HttpResponseNotFound(pagenotfound_msg)
    next_day = date + timedelta(hours=23.99)
    days_tasks = TaskTracking.objects.filter(
        start_time__range=(
            date, next_day)).filter(
        user=request.user)
